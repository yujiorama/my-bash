...
mkdir -p "${MY_BASH_APP}/docker" "${MY_BASH_APP}/docker/certs"

if [[ ! -e "${MY_BASH_APP}/docker/env" ]]; then

    if [[ ! -e "${MY_BASH_APP}/minikube/docker/env" ]] && [[ ! -d "${MY_BASH_APP}/minikube/docker/certs" ]]; then
